...
// Removed: const { insertData, renameAndCreateTable } = require('./mysqlHandler');
const { createInitialKML, updateKML } = require('./kmlHandler');
const { updateBackup } = require('./backupHandler');
const fs = require('fs');
const path = require('path');

const portName = "COM4"; // Change this to your Arduino's serial port
const baudRate = 9600;
const csvFile = path.join(__dirname, '..', 'sheet.csv');
const kmlFile = path.join(__dirname, '..', 'live_track.kml');

const csvHeaders = [
  "Time", "Temperature", "Pressure", "Altitude", "Latitude", "Longitude",
  "gps_altitude", "gps_sats", "gyro_x", "gyro_y", "gyro_z",
  "bmp_status", "gps_status", "gyro_status", "apc_status", "servo_status",
  "servo_rotation", "sd_status"
];
...
function startSerialReading() {
  ...
  // Removed: renameAndCreateTable() call
  
  // Create a SerialPort instance with options (path, baudRate, autoOpen)
  const port = new SerialPort({ path: portName, baudRate: baudRate, autoOpen: false });
  ...
  parser.on('data', async (line) => {
    ...
    if (parsed && parsed.key) {
      // When a new dataset begins (detected by a new "Time" field)
